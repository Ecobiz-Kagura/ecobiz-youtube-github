...
param(
  ...
  # mp4無ければ自動生成（Switchはデフォfalseなので、既定ONにしたいなら -NoAutoMakeMp4 を用意するのが王道だが、ここでは簡潔にboolで）
  [Parameter(Mandatory=$false)]
...
function Build-UploaderCmd(
  ...
  $argsAll += @("--mode",$mode)
  $argsAll += @("--mp4",$mp4Path)
  $argsAll += @("--prefix"," ")   # 空は argparse が死ぬ
...
# =====================================================
$cwd = (Get-Location).Path
$mp4s = @(Get-ChildItem -LiteralPath $cwd -Filter *.mp4 -File -ErrorAction SilentlyContinue)

if($mp4s.Count -gt 0){
  Write-Host "=== 注意：mp4 削除 ==="
  Write-Host "以下の mp4 を削除します："
  $mp4s | ForEach-Object { Write-Host " - $($_.FullName)" }
  Write-Host "========================"
...
$ps1Mp4Cand = Join-Path $ps1Dir ($ps1Stem + ".mp4")
...
$baseNorm =
  ...
  "pad=${TargetW}:${TargetH}:(ow-iw)/2:(oh-ih)/2[base0]"

if($OverlayMode -eq "video"){
  $ovNorm = if($Alpha -lt 1.0){
    "[1:v]scale=${TargetW}:${TargetH}:force_original_aspect_ratio=decrease," +
    "pad=${TargetW}:${TargetH}:(ow-iw)/2:(oh-ih)/2,format=rgba," +
    "colorchannelmixer=aa=${Alpha}[ov0]"
  } else {
    "[1:v]scale=${TargetW}:${TargetH}:force_original_aspect_ratio=decrease," +
    "pad=${TargetW}:${TargetH}:(ow-iw)/2:(oh-ih)/2[ov0]"
  }
