...
# === filter_complex ===
# 横幅 = 背景の50% → 高さも同じ → 正方形に変形
# 右上配置: x=main_w-w-Margin, y=Margin
$filter = "[1:v][0:v]scale2ref=w=main_w*0.5:h=main_w*0.5[ovl][base];" +
          "[ovl]format=yuva420p,colorchannelmixer=aa=${OverlayAlpha}[ovla];" +
          "[base][ovla]overlay=x=main_w-w-${Margin}:y=${Margin}:format=auto[vout]"
